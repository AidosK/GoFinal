...
import (
	...
	"github.com/AidosK/GoFinal/internal/driver"
	"github.com/AidosK/GoFinal/internal/forms"
	"github.com/AidosK/GoFinal/internal/helpers"
	"github.com/AidosK/GoFinal/internal/models"
	"github.com/AidosK/GoFinal/internal/render"
	"github.com/AidosK/GoFinal/internal/repository"
	"github.com/AidosK/GoFinal/internal/repository/dbrepo"
...
func (m *Repository) PostSignUp(w http.ResponseWriter, r *http.Request) {
	...
	form.MinLength("password", 4, 12)

	if !form.Valid() {
		data := make(map[string]interface{})
		data["title"] = "Sign Up"
		data["path"] = "/user/signup"
		data["user"] = user
	...
	m.App.Session.Put(r.Context(), "flash", "Sign Up successful")
	http.Redirect(w, r, "/user/login", http.StatusSeeOther)
}
